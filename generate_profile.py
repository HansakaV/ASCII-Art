...
INFO = [
    ("__header__", "Mahesh Hansaka", ""),
    ("__rule__", "", ""),
    ("Role",      "Associate Software Engineer | Full-Stack Developer", "val"),
    ("Edu",       "BSc (Hons) Computer Science (Reading) · Univ of Wolverhampton | Higher National Diploma in Software Engineering · IJSE", "val"),
    ("Focus",     "Full-Stack Engineering · AI/ML Systems · Cloud", "accent"),
    ("__blank__", "", ""),
    ("__section__", "~/stack", ""),
    ("Lang",      "Java · C# · TypeScript · JavaScript · Python · SQL", "val"),
    ("Backend",   "Spring Boot · Node.js · FastAPI · .NET Core · REST APIs", "val"),
    ("Cloud",     "Render · AWS · GCP · Docker · Kubernetes · CI/CD", "val"),
    ("Data",      "MongoDB · PostgreSQL · MySQL· FireBase · SupaBase", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/projects", ""),
    ("BusDepot",  "Depot Management System · Spring Boot & GCP", "warn"),
    ("MediCare",  "Healthcare Platform with e-SMS gateway", "warn"),
    ("Crypto",    "BTC Price Trend Forecasting Dashboard (XGBoost)", "warn"),
    ("Coreflow",  "Network Diagnostics Tool · FastAPI & Next.js", "warn"),
    ("__blank__", "", ""),
    ("__section__", "~/highlights", ""),
    ("Experience", "8 Months Industry Intern + 2+ Years Freelance", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/reach", ""),
    ("Web",       "new-portfolio-hasa.vercel.app", "accent"),
    ("In",        "linkedin.com/in/mahesh-hansaka-1069a3310", "accent"),
    ("Mail",      "maheshhansaka628@gmail.com", "accent"),
]
...
def render(theme_name, colors, stats, ist_now):
    ...
    box_h = H - 2
    parts.append(
        f'<rect x="1" y="1" width="{box_w}" height="{box_h}" rx="12" '
        f'fill="{colors["bg"]}" stroke="{colors["border"]}" stroke-width="1.5"/>'
    )
    
    path_w = W - 26
    parts.append(
        f'<path d="M1 13 a12 12 0 0 1 12 -12 h{path_w} a12 12 0 0 1 12 12 v25 h{-(W-2)} z" '
        f'fill="{colors["panel"]}"/>'
    )
    
    line_w = W - 1
    parts.append(f'<line x1="1" y1="38" x2="{line_w}" y2="38" stroke="{colors["border"]}"/>')
    for i, c in enumerate(["dot1", "dot2", "dot3"]):
        parts.append(f'<circle cx="{24 + i*20}" cy="20" r="6" fill="{colors[c]}"/>')
        
    mid_w = W / 2
    parts.append(
        f'<text x="{mid_w}" y="24" class="ttl" text-anchor="middle">'
    ...
    for label, value, ckey in INFO:
        ...
        if label == "__header__":
            ...
            line_x2 = W - 40
            parts.append(
                f'<line x1="{INFO_X}" y1="{y-8:.1f}" x2="{line_x2}" y2="{y-8:.1f}" '
    ...
    parts.append(
        ...
        f'<tspan class="key">➜</tspan>'
        f'<tspan class="acc" dx="8">~</tspan>'
        f'<tspan class="val" dx="8">open to Full-Stack / Backend / DevOps  roles</tspan>'
        f'<tspan class="cur" dx="8">█</tspan></text>'
    )
    
    text_w = W - 34
    parts.append(
        f'<text x="{text_w}" y="{fy}" class="mut" text-anchor="end">'
